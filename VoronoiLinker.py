...
def DrawLine(ps1, ps2, sz=1, cl1=(1.0, 1.0, 1.0, 0.75), cl2=(1.0, 1.0, 1.0, 0.75), fs=[0, 0]): DrawWay(
    ((ps1[0] + fs[0], ps1[1] + fs[1]), (ps2[0] + fs[0], ps2[1] + fs[1])), (cl1, cl2), sz)


def DrawCircleOuter(pos, rd, siz=1, col=(1.0, 1.0, 1.0, 0.75), resolution=16):
    vtxs = [];
    vcol = []
    for cyc in range(resolution + 1): vtxs.append(
        (rd * cos(cyc * 2 * pi / resolution) + pos[0], rd * sin(cyc * 2 * pi / resolution) + pos[1])); vcol.append(col)
    DrawWay(vtxs, vcol, siz)


def DrawCircle(pos, rd, col=(1.0, 1.0, 1.0, 0.75), resl=54): DrawAreaFan([(pos[0], pos[1]), *[
    (rd * cos(i * 2 * pi / resl) + pos[0], rd * sin(i * 2 * pi / resl) + pos[1]) for i in range(resl + 1)]], col, True)
...
def DrawRectangle(ps1, ps2, cl): DrawAreaFan([(ps1[0], ps1[1]), (ps2[0], ps1[1]), (ps2[0], ps2[1]), (ps1[0], ps2[1])],
                                             cl, False)
...
def DrawText(pos, ofsx, ofsy, txt, draw_col):
    ...
    if GetAddonPrefs().ds_text_style == 'Classic':
        for cyc in range(len(list)): DrawRectangle([pos1[0], pos1[1] + cyc * uh], [pos2[0], pos1[1] + cyc * uh + uh],
...
def GetAddonPrefs(): return bpy.context.preferences.addons[
    __name__ if __name__ != '__main__' else 'VoronoiLinker'].preferences


def SetFont(): gv_font_id[0] = blf.load(r'C:\Windows\Fonts\consola.ttf'); gv_font_id[0] = 0 if gv_font_id[0] == -1 else \
gv_font_id[0]  # for change Blender themes


def PosViewToReg(x, y): return bpy.context.region.view2d.view_to_region(x, y, clip=False)


def PreparGetWP(loc, offsetx): pos = PosViewToReg(loc.x + offsetx, loc.y); rd = \
...
def DebugDrawCallback(sender, context):
    def DrawDbText(pos, txt, r=1, g=1, b=1):
        blf.size(gv_font_id[0], 14, 72); blf.position(gv_font_id[0], pos[0] + 10, pos[1], 0); blf.color(gv_font_id[0],
                                                                                                        r, g, b,
                                                                                                        1.0); blf.draw(
            gv_font_id[0], txt)
    ...
    for li in list_nodes:
        if li[1].type != 'FRAME': wp = PreparGetWP(li[2], 0); DrawWidePoint(wp[0], wp[1],
                                                                            Vector((1, .5, .5, 1))); DrawDbText(wp[0],
                                                                                                                str(sco) + ' Node goal here',
                                                                                                                g=.5,
                                                                                                                b=.5); sco += 1
    list_socket_in, list_socket_out = GenNearestSocketsList(list_nodes[0][1], mouse_pos)
    if list_socket_out: wp = PreparGetWP(list_socket_out[0][2], 0); DrawWidePoint(wp[0], wp[1],
                                                                                  Vector((.5, .5, 1, 1))); DrawDbText(
        wp[0], 'Nearest socketOut here', r=.75, g=.75)
    if list_socket_in: wp = PreparGetWP(list_socket_in[0][2], 0); DrawWidePoint(wp[0], wp[1],
                                                                                Vector((.5, 1, .5, 1))); DrawDbText(
...
def GenNearestNodeList(nodes,
    ...
    def ToSign(vec2): return Vector(
        (copysign(1, vec2[0]), copysign(1, vec2[1])))  # Для запоминания своего квадранта перед abs().
    ...
    def GetFromPut(side_mark, who_puts):
        ...
        for wh in who_puts:
            ...
            if (wh.enabled) and (wh.hide == False):
                ...
                list_whom.append([(pick_pos - sk_loc_car).length, wh, goal_pos, (
                goal_pos.y - 11 - muv * 20, goal_pos.y + 11 + max(length(wh.links) - 2, 0) * 5 * (side_mark == -1))])
...
def VoronoiLinkerDrawCallback(sender, context):
    ...
    def LinkerDrawSk(Sk):
        ...
        if Sk.is_linked: DrawIsLinked(mouse_pos, -txtdim[0] * (Sk.is_output * 2 - 1), 0,
    ...
    if (sender.list_sk_goal_out == []):
        ...
        if (GetAddonPrefs().vlds_is_always_line) and (GetAddonPrefs().ds_is_draw_line): DrawLine(wp1[0], wp2[0], lw,
                                                                                                 (1, 1, 1, 1),
                                                                                                 (1, 1, 1, 1))
        ...
        if GetAddonPrefs().ds_is_draw_point: DrawWidePoint(wp1[0], wp1[1],
                                                           GetSkVecCol(sender.list_sk_goal_out[1], 2.2)); DrawWidePoint(
            wp2[0], wp2[1])
        ...
        if GetAddonPrefs().ds_is_colored_line:
            col1 = GetSkCol(sender.list_sk_goal_out[1]); col2 = GetSkCol(sender.list_sk_goal_in[1])
        else:
            col1 = (1, 1, 1, 1); col2 = (1, 1, 1, 1)
        wp1 = PreparGetWP(sender.list_sk_goal_out[2] * gv_uifac[0], GetAddonPrefs().ds_point_offset_x)
                ...
                if (event.value == 'RELEASE') and (self.list_sk_goal_out) and (self.list_sk_goal_in):
                    ...
                    if self.list_sk_goal_in[
                        ...
                        for lk in self.list_sk_goal_in[1].links: list_sk_links.append(
                            (lk.from_socket, lk.to_socket)); tree.links.remove(lk)
...
def VoronoiMassLinkerDrawCallback(sender, context):
    ...
    if (sender.nd_goal_out == None):
        ...
        for lsk in list_Sks:
            ...
            if (GetAddonPrefs().vlds_is_always_line) and (GetAddonPrefs().ds_is_draw_line):
                DrawLine(wp1[0], wp2[0], lw, GetSkCol(lsk[1]) if GetAddonPrefs().ds_is_colored_line else (1, 1, 1, 1),
                         (1, 1, 1, 1))
            if GetAddonPrefs().ds_is_draw_point: DrawWidePoint(wp1[0], wp1[1], GetSkVecCol(lsk[1], 2.2)); DrawWidePoint(
                wp2[0], wp2[1])
    else:
        ...
        for lsks in sender.list_equalSks:
            ...
            if GetAddonPrefs().ds_is_colored_line:
                col1 = GetSkCol(lsks[0][1]); col2 = GetSkCol(lsks[1][1])
            else:
                col1 = (1, 1, 1, 1); col2 = (1, 1, 1, 1)
            ...
            if GetAddonPrefs().ds_is_draw_line: DrawLine(wp1[0], wp2[0], lw, col1, col2)
            if GetAddonPrefs().ds_is_draw_point: DrawWidePoint(wp1[0], wp1[1],
                                                               GetSkVecCol(lsks[0][1], 2.2)); DrawWidePoint(wp2[0],
                                                                                                            wp2[1],
                                                                                                            GetSkVecCol(
                                                                                                                lsks[1][
                                                                                                                    1],
...
def VoronoiMixerDrawCallback(sender, context):
    ...
    def MixerDrawSk(Sk, ys, lys):
        ...
        if Sk.is_linked: DrawIsLinked(mouse_pos, txtdim[0], txtdim[1] * lys * .75,
                                      GetSkCol(Sk) if GetAddonPrefs().ds_is_colored_marker else (.9, .9, .9, 1))
    ...
    if (sender.list_sk_goal_out1 == []):
        ...
        if GetAddonPrefs().ds_is_draw_point: DrawWidePoint(wp1[0], wp1[1], GetSkVecCol(sender.list_sk_goal_out1[1],
                                                                                       2.2)); DrawWidePoint(wp2[0],
                                                                                                            wp2[1])
        ...
        if GetAddonPrefs().ds_is_colored_line:
            col1 = GetSkCol(sender.list_sk_goal_out1[1]); col2 = GetSkCol(sender.list_sk_goal_out2[1])
        else:
            col1 = (1, 1, 1, 1); col2 = (1, 1, 1, 1)
        wp1 = PreparGetWP(sender.list_sk_goal_out1[2] * gv_uifac[0], GetAddonPrefs().ds_point_offset_x)
        wp2 = PreparGetWP(sender.list_sk_goal_out2[2] * gv_uifac[0], GetAddonPrefs().ds_point_offset_x)
        if GetAddonPrefs().ds_is_draw_line: DrawLine(mouse_region_pos, wp2[0], lw, col2, col2); DrawLine(wp1[0],
                                                                                                         mouse_region_pos,
                                                                                                         lw, col1, col1)
        if GetAddonPrefs().ds_is_draw_point:
...
mixerSkTyp = [None]
dict_mixer_defs = {
    'GeometryNodeSwitch': [-1, -1, 'Switch'], 'ShaderNodeMixShader': [1, 2, 'Mix'],
    'ShaderNodeAddShader': [0, 1, 'Add'], 'ShaderNodeMixRGB': [1, 2, 'Mix RGB'],
    'ShaderNodeMath': [0, 1, 'Max'], 'ShaderNodeVectorMath': [0, 1, 'Max'], 'FunctionNodeBooleanMath': [0, 1, 'Or'],
    'FunctionNodeCompare': [-1, -1, 'Compare'],
    'GeometryNodeCurveToMesh': [0, 1, 'Curve to Mesh'], 'GeometryNodeInstanceOnPoints': [0, 2, 'Instance on Points'],
    'GeometryNodeMeshBoolean': [0, 1, 'Boolean'],
    'GeometryNodeStringJoin': [1, 1, 'Join'], 'GeometryNodeJoinGeometry': [0, 0, 'Join'],
    ...
    'CompositorNodeSplitViewer': [0, 1, 'Split Viewer'], 'CompositorNodeSwitchView': [0, 1, 'Switch View'],
...
dictMixerMain = {
    ...
    'GeometryNodeTree': {'VALUE': ['GeometryNodeSwitch', 'ShaderNodeMixRGB', 'FunctionNodeCompare', 'ShaderNodeMath'],
                         ...
                         'BOOLEAN': ['GeometryNodeSwitch', 'ShaderNodeMixRGB', 'ShaderNodeMath',
                                     'FunctionNodeBooleanMath'], 'OBJECT': ['GeometryNodeSwitch'],
                         'MATERIAL': ['GeometryNodeSwitch'], 'COLLECTION': ['GeometryNodeSwitch'],
                         'TEXTURE': ['GeometryNodeSwitch'], 'IMAGE': ['GeometryNodeSwitch']},
    'CompositorNodeTree': {'VALUE': ['CompositorNodeMixRGB', 'CompositorNodeSwitch', 'CompositorNodeSplitViewer',
                           ...
                           'INT': ['CompositorNodeMixRGB', 'CompositorNodeSwitch', 'CompositorNodeSplitViewer',
                                   'CompositorNodeSwitchView', 'CompositorNodeMath']},
...
class VoronoiMixerMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        for li in dictMixerMain[context.space_data.tree_type][mixerSkTyp[0]]: who.operator('node.voronoi_mixer_mixer',
                                                                                           text=dict_mixer_defs[li][
                                                                                               2]).who = li
    ...
    if (sender.list_sk_goal_out == []) or (sender.list_sk_goal_out[1] == None):  # Второе условие -- для (1).
        ...
        def PreviewerDrawSk(Sk):
            txtdim = DrawSkText(PosViewToReg(mouse_pos.x, mouse_pos.y), GetAddonPrefs().ds_text_dist_from_cursor, -.5,
                                Sk)
            if Sk.is_linked: DrawIsLinked(mouse_pos, txtdim[0], 0,
                                          GetSkCol(Sk) if GetAddonPrefs().ds_is_colored_marker else (.9, .9, .9, 1))
    ...
    def NextAssign(sender, context):
        ...
        for li in list_nodes:
            ...
            if tgl:
                ...
                break
        if (GetAddonPrefs().vp_is_live_preview) and (sender.list_sk_goal_out): sender.list_sk_goal_out[
            1] = VoronoiPreviewer_DoPreview(context, sender.list_sk_goal_out[1])
        ...
        match event.type:
            ...
            case 'LEFTMOUSE' | 'RIGHTMOUSE' | 'ESC':
                ...
                if (event.value == 'RELEASE') and (self.list_sk_goal_out):
                    self.list_sk_goal_out[1] = VoronoiPreviewer_DoPreview(context, self.list_sk_goal_out[1]); return {
                        'FINISHED'}
                else:
                    return {'CANCELLED'}
        return {'RUNNING_MODAL'}
...
def VoronoiPreviewer_DoPreview(context, goalSk):
    ...
    for cyc in range(hig_way + 1):
        if (list_way_trnd[cyc][1] == None) and (
                cyc > 0): continue  # Проверка по той же причине, по которой мне не нравился способ от NW.
        node_in = None;
        sock_out = None;
        sock_in = None
        ...
        if cyc != hig_way:
            for nd in list_way_trnd[cyc][0].nodes:
                if (nd.type in ['GROUP_OUTPUT', 'OUTPUT_MATERIAL', 'OUTPUT_WORLD', 'OUTPUT_LIGHT', 'COMPOSITE',
                                'OUTPUT']) and (nd.is_active_output): node_in = nd
        else:
            match context.space_data.tree_type:
                case 'ShaderNodeTree':
                ...
                case 'CompositorNodeTree':
                    for nd in list_way_trnd[hig_way][0].nodes: sock_in = nd.inputs[0] if (
                                nd.type == 'VIEWER') else sock_in
                    if sock_in == None:
                        for nd in list_way_trnd[hig_way][0].nodes: sock_in = nd.inputs[0] if (
                                    nd.type == 'COMPOSITE') else sock_in
                case 'GeometryNodeTree':
                    for nd in list_way_trnd[hig_way][0].nodes:
        ...
        if cyc == 0:
            ...
            if (sock_out == None) and (ix_sk_last_used in range(0, length(list_way_trnd[cyc][1].outputs))): sock_out = \
            list_way_trnd[cyc][1].outputs[ix_sk_last_used]
        ...
        if nd_va:
            list_way_trnd[cyc][0].links.new(sock_out, nd_va.inputs[
                0]); break  # Завершение после напарывания повышает возможности использования якоря.
    ...
    if sender.is_target_node:
        if (sender.list_nd_goal == []):
            ...
            if GetAddonPrefs().vh_draw_text_for_unhide:
                ...
                if lbl: DrawText(PosViewToReg(mouse_pos.x, mouse_pos.y), GetAddonPrefs().ds_text_dist_from_cursor,
                                 l_ys[1], lbl, (1, 1, 1, 1))
    else:
...
class VoronoiHider(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        match event.type:
            ...
            case 'ESC':
                bpy.types.SpaceNodeEditor.draw_handler_remove(self.dcb_handle, 'WINDOW'); return {'CANCELLED'}
            case 'E':
...
class FastMath_Main(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        def DispMenu(dp):
            displayDeep[0] = dp; bpy.ops.wm.call_menu_pie(name='VL_MT_voronoi_fastmath_pie')
        ...
        if self.bridge in ['', ' ']:
            ...
            displayList[0] = [li[1] for li in whoList if li[0] == self.bridge][0]; DispMenu(1)
        else:
...
def register():
    ...
    for (bl_id, key, Shift, Ctrl, Alt) in kmi_defs: kmi = km.keymap_items.new(idname=bl_id, type=key, value='PRESS',
                                                                              shift=Shift, ctrl=Ctrl,
                                                                              alt=Alt); list_addon_keymaps.append(
        (km, kmi))
