...
### END LICENSE BLOCK
bl_info = {'name':'Voronoi Linker','author':'ugorek','version':(1,5,0),'blender':(3,4,0), #15.12.2022
...
def VoronoiPreviewerDrawCallback(sender,context):
    ...
    if (sender.sockOutSk==None):
        ...
        if GetDrawSettings('DrLn'): DrawLine(wp[0],mouseRegionPs,1,col,col)
...
class VoronoiAddonPrefs(bpy.types.AddonPreferences):
    ...
    def draw(self,context):
        col0 = self.layout.column(); box = col0.box(); col1 = box.column(align=True); col1.label(text='Draw stiings'); col1.prop(self,'PtOX'); row = col1.row(align=True)
        ...
        box = col0.box(); col1 = box.column(align=True); col1.label(text='Preview stiings'); col1.prop(self,'LvPr'); col1.prop(self,'PrGm')
