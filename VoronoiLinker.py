...
def SetFont():
    ...
    gv_font_id[0] = 0 if gv_font_id[0] == -1 else gv_font_id[0]  # for change Blender themes
...
def PreparGetWP(loc, offsetx):
    ...
    rd = PosViewToReg(loc.x + offsetx + 6 * GetAddonPrefs().ds_point_radius, loc.y)[0] - pos[0]
    return pos, rd
...
def GenNearestSocketsList(nd, pick_pos):
    # Выдаёт список "ближайших сокетов". Честное поле расстояний ячейками Вороного.
    list_socket_in = []
    list_socket_out = []
        ...
        for wh in who_puts:
            ...
            if (wh.enabled) and (wh.hide == False):
                ...
                list_whom.append([(pick_pos - sk_loc_car).length, wh, goal_pos, (
                    goal_pos.y - 11 - muv * 20,
                    goal_pos.y + 11 + max(length(wh.links) - 2, 0) * 5 * (side_mark == -1))])
...
def VoronoiLinkerDrawCallback(sender, context):
    ...
    if GetAddonPrefs().ds_is_draw_debug:
        DebugDrawCallback(sender, context)
        return
    mouse_pos = context.space_data.cursor_location * gv_uifac[0]
...
def VoronoiMixerDrawCallback(sender, context):
    ...
    if (sender.list_sk_goal_out1 == []):
        ...
        if GetAddonPrefs().ds_is_draw_line:
            DrawLine(mouse_region_pos, wp2[0], lw, col2, col2)
            DrawLine(wp1[0], mouse_region_pos, lw, col1, col1)
